...
def plot_oscillator_step(sqr_err_df : pd.DataFrame, filename : str = "oscillator_step.png", xlim : tuple[float, float] = None, ylim : tuple[float, float] = None, hue : str = None) -> None:
    ...
    if hue is None:
        sns.lineplot(data=sqr_err_df, x="step", y="sqr_err", marker="o", errorbar=None, color="blue", label="")
    else:
        sns.lineplot(data=sqr_err_df, x="step", y="sqr_err", marker="o", errorbar=None, hue=hue)
