...
def plot_polarization_over_iterations(simulations_info : list[SimulationInfo], plot_file : str, colors : list[str], noises : list[float]):
    ...
    plt.plot([], [], ' ', label="η = {}".format(str(noises[0]) + "π"))
    ...
    for simulation_info in simulations_info:
        ...
        add_polarization_over_iterations_plot(instants, polarization_values, colors[i], str(simulations_info[i].L) + " - " + "{:.2g}".format(int(simulations_info[i].N) / (int(simulations_info[i].L) ** 2)))
        i += 1
    plt.ylim((0, 1.05))
